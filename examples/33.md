...
21 |     let z = bar2.f(); // #3 error: cannot borrow `bar2` as mutable more than once at a time [E0499]
   ...
   |             first borrow later used here
   |
   = note: Consider the following approaches to work around this limitation:
   ...
   6. Reorganize your code to split functions into smaller pieces that operate on different parts of the data.
   7. Redesign your data structures to minimize the need for multiple mutable references.
